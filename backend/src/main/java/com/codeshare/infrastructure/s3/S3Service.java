...
public class S3Service {
    ...
    public S3Service(@Value("${s3.region}") String region, @Value("${s3.enabled:false}") boolean enabled) {
    ...
    public List<String> listSnapshotsForRoom(String roomId) {
        ...
        try {
            ListObjectsV2Request req = new ListObjectsV2Request().withBucketName(bucket).withPrefix(roomId + "/");
            List<String> keys = s3.listObjectsV2(req).getObjectSummaries().stream()
