...
class FlywayMigrationTest {
    ...
    void testRoomMembersTableStructure() {
        ...
        assertTrue(columnNames.contains("member_id"), "Room_members table should have member_id column");
    }
    ...
    void testIndexesExist() {
        ...
        assertTrue(indexNames.stream().anyMatch(name -> name.contains("users_email")), 
                "Users email index should exist");
        assertTrue(indexNames.stream().anyMatch(name -> name.contains("rooms_owner_id")), 
                "Rooms owner_id index should exist");
        assertTrue(indexNames.stream().anyMatch(name -> name.contains("room_members_room_id")), 
                "Room_members room_id index should exist");
        assertTrue(indexNames.stream().anyMatch(name -> name.contains("room_members_member_id")), 
    ...
    void testTriggersExist() {
        ...
        List<String> triggerNames = triggers.stream()
                .map(trigger -> (String) trigger.get("trigger_name"))
                .toList();
        
        assertTrue(triggerNames.contains("update_users_updated_at"), 
                "Users updated_at trigger should exist");
        assertTrue(triggerNames.contains("update_rooms_updated_at"), 
